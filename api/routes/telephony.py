...
async def websocket_ari_endpoint(websocket: WebSocket):
    ...
    # Check if workflow run exists in local DB to determine if it is a local or cloud call
    from api.db import db_client
    import aiohttp
    import asyncio
    ...
    workflow_run = None
    try:
        workflow_run = await db_client.get_workflow_run(int(workflow_run_id))
    except Exception:
        pass

    if not workflow_run:
        logger.info(
            f"[ARI Proxy] Workflow run {workflow_run_id} not found in local DB. "
            f"Relaying secure WebSocket stream to app.dograh.com..."
        )
        await websocket.accept(subprotocol="media")
        
        cloud_url = f"wss://api.dograh.com/api/v1/telephony/ws/ari?workflow_id={workflow_id}&user_id={user_id}&workflow_run_id={workflow_run_id}"
        
        try:
            async with aiohttp.ClientSession() as session:
                async with session.ws_connect(cloud_url, protocols=["media"]) as ws_cloud:
                    
                    async def forward_to_cloud():
                        try:
                            logger.info("[ARI Proxy] Starting forward_to_cloud loop")
                            while True:
                                data = await websocket.receive()
                                if "bytes" in data:
                                    await ws_cloud.send_bytes(data["bytes"])
                                elif "text" in data:
                                    await ws_cloud.send_str(data["text"])
                                elif data.get("type") == "websocket.disconnect":
                                    logger.info("[ARI Proxy] Client disconnected in forward_to_cloud")
                                    break
                        except Exception as e:
                            logger.error(f"[ARI Proxy] Error in forward_to_cloud: {e}")

                    async def forward_to_client():
                        try:
                            logger.info("[ARI Proxy] Starting forward_to_client loop")
                            async for msg in ws_cloud:
                                if msg.type == aiohttp.WSMsgType.BINARY:
                                    ...
                                    logger.info(f"[ARI Proxy] Cloud connection closed: {msg.type}")
                                    break
                        except Exception as e:
                            logger.error(f"[ARI Proxy] Error in forward_to_client: {e}")

                    await asyncio.gather(forward_to_cloud(), forward_to_client())
        except Exception as e:
            logger.error(f"[ARI Proxy] Proxy session failed: {e}")
        return
