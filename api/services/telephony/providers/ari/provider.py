    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        if to_number.startswith("SIP/") or to_number.startswith("PJSIP/"):
            ...
            # If the number is a PSTN phone number (e.g. >= 10 digits)
            if len(clean_number) >= 10:
                # Automatically route through Vobiz trunk!
                sip_endpoint = f"vobiz-out/sip:{clean_number}@31868684.sip.vobiz.ai"
            else:
    ...
    async def transfer_call(
        ...
        **kwargs: Any,
        ...
        if destination.startswith("SIP/") or destination.startswith("PJSIP/"):
            ...
            clean_number = "".join(c for c in destination if c.isdigit())
            if len(clean_number) >= 10:
                sip_endpoint = f"PJSIP/vobiz-out/sip:{clean_number}@31868684.sip.vobiz.ai"
            else:
