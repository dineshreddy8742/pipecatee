...
class ARIConnection:
    ...
    async def _create_bridge_and_add_channels(self, channel_ids: list) -> str:
        """Create a bridge and add channels to it with a retry loop for tunnel handshakes."""
        # Create bridge
        ...
        # Add channels to bridge with a retry loop to account for network/tunnel latency
        for attempt in range(15):
            response = await self._ari_request(
                "POST",
                f"/bridges/{bridge_id}/addChannel",
                params={"channel": ",".join(channel_ids)},
            )
            # If _ari_request succeeded, it will return a non-empty dictionary or list
            if response:
                logger.info(
                    f"[ARI org={self.organization_id}] Bridge {bridge_id} created with channels: {channel_ids} (attempt {attempt+1})"
                )
                return bridge_id
            
            logger.warning(
                f"[ARI org={self.organization_id}] Channel not ready for bridge yet (attempt {attempt+1}), retrying in 200ms..."
            )
            await asyncio.sleep(0.2)

        logger.error(
            f"[ARI org={self.organization_id}] Failed to add channels to bridge after all attempts"
        )
        return ""
